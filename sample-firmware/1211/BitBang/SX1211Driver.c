...
_U16 RegistersCfg[] = { // SX1211 configuration registers values
		...
		DEF_MCPARAM2 | RF_MC2_MODULATION_FSK | RF_MC2_DATA_MODE_CONTINUOUS | RF_MC2_OOK_THRESH_TYPE_PEAK | RF_MC2_GAIN_IF_00,                      
...
void InitRFChip (void){
    ...
    for(i = 0; i < SyncSize; i++){
        SyncValue[i] = InvertByte(RegistersCfg[REG_SYNCBYTE1 - 1 + i]);
    }
    ...
    //100kbps not supported in continuous mode (CPU speed limitation)
    else { 
        RFFrameTimeOut = RF_FRAME_TIMEOUT(1600);
    }
...
void ReceiveRfFrame(_U8 *buffer, _U8 *size, _U8 *pReturnCode){
    ...
    }while((RegPAIn & IRQ_0) == 0x00);      // Waits The Sync word detection of the SX1211
