...
**               XE8000 family products !!! Buffered mode !!!     **
*******************************************************************/
...
_U16 RegistersCfg[] = { // SX1212 configuration registers values
		...
		DEF_MCPARAM2 | RF_MC2_DATA_MODE_BUFFERED | RF_MC2_MODULATION_FSK | RF_MC2_OOK_THRESH_TYPE_PEAK | RF_MC2_GAIN_IF_00,                      
		...
		DEF_FIFOCONFIG | RF_FIFO_SIZE_16 | RF_FIFO_THRESH_VALUE,
		DEF_IRQPARAM1 | RF_IRQ0_RX_STDBY_OFF | RF_IRQ1_RX_STDBY_OFF | RF_IRQ0_TX_FIFOEMPTY_START_FIFONOTEMPTY | RF_IRQ1_TX_FIFOFULL,                     
		DEF_IRQPARAM2 | RF_IRQ2_FIFOFILL_METHOD_AUTO | RF_IRQ2_FIFOFILL_FLAGCTRL_0 | RF_IRQ2_PLL_LOCK_PIN_ON,                     
		...
		DEF_RXPARAM3 | RF_RX3_POLYPFILT_OFF | RF_RX3_SYNC_RECOG_ON | RF_RX3_SYNC_SIZE_32 | RF_RX3_SYNC_TOL_0,                      
...
void SendRfFrame(_U8 *buffer, _U8 size, _U8 *pReturnCode){
    ...
    WriteRegister(REG_IRQPARAM1, RF_IRQ1_TX_FIFOFULL);
    ...
    for(ByteCounter = 0; ByteCounter < SyncSize; ){
        if(!(RegPAIn & IRQ_1)){ // If FIFO not full send a new byte
    ...
    // Select TX IRQ_1 source to TX_done
    WriteRegister(REG_IRQPARAM1, RF_IRQ1_TX_TXDONE);
...
void ReceiveRfFrame(_U8 *buffer, _U8 *size, _U8 *pReturnCode){
    ...
    if(TempRFState & RF_STOP){
        ...
        WriteRegister(REG_IRQPARAM1, RegistersCfg[REG_IRQPARAM1] | RF_IRQ0_RX_STDBY_WRITEBYTE | RF_IRQ1_FIFO_OVERRUN_CLEAR);
        WriteRegister(REG_IRQPARAM2, RegistersCfg[REG_IRQPARAM2] | RF_IRQ2_FIFOFILL_FLAGCTRL_1);
...
/*******************************************************************
** SX1212 Buffered interrupt handlers                             **
*******************************************************************/
